use emmylua_code_analysis::{DbIndex, LuaAliasCallKind, LuaMemberInfo, LuaMemberKey, LuaSemanticDeclId, LuaType, SemanticModel, get_keyof_members, try_extract_signature_id_from_field, get_real_type};
use emmylua_parser::{
    LuaAssignStat, LuaAstNode, LuaAstToken, LuaFuncStat, LuaGeneralToken, LuaIndexExpr,
    LuaParenExpr, LuaTokenKind,
};
use emmy_lsp_types::CompletionItem;
...
use super::{CallDisplay, check_visibility, get_completion_kind, get_description, get_detail, is_deprecated, CompletionTriggerStatus};
...
pub fn add_member_completion(
    ...
    let mut completion_item = CompletionItem {
        ...
        label_details: Some(emmy_lsp_types::CompletionItemLabelDetails {
            detail,
            description,
        }),
        deprecated,
        ...
        && builder.trigger_token.kind() == LuaTokenKind::TkDot.into()
    {
        ...
        completion_item.additional_text_edits = Some(vec![emmy_lsp_types::TextEdit {
    ...
    if can_add_snippet && builder.support_snippets(typ) {
        if let Some(snippet) = get_function_snippet(builder, &label, typ, call_display) {
            ...
            completion_item.insert_text_format = Some(emmy_lsp_types::InsertTextFormat::SNIPPET);
        }
    }
...
fn try_add_alias_completion_item_new(
    ...
    let index_hint = ("index ".to_owned() + label).to_string();
