...
use emmy_lsp_types::{CompletionItem, Documentation, MarkedString, MarkupContent};
...
pub fn update_function_signature_info(
        ...
        && overload_count > 0
    {
        if let Some(signature_overload) = &mut hover_builder.signature_overload {
            for signature in (&mut **signature_overload).iter_mut() {
...
fn build_vscode_completion_item(
    ...
    let type_description = overload_index
        .and_then(|index| {
            hover_builder
                .signature_overload
                .and_then(|overloads: Vec<MarkedString>| overloads.get(index).cloned())
        })
