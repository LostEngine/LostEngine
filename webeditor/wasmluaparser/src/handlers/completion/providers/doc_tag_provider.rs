...
use emmy_lsp_types::{CompletionItem, MarkupContent};
...
fn add_tag_completion(builder: &mut CompletionBuilder, sorted_index: usize, tag: &str) {
    let completion_item = CompletionItem {
        label: tag.to_string(),
        kind: Some(emmy_lsp_types::CompletionItemKind::EVENT),
        documentation: Some(emmy_lsp_types::Documentation::MarkupContent(MarkupContent {
            kind: emmy_lsp_types::MarkupKind::Markdown,
...
fn add_tag_param_return_completion(
    ...
    let completion_item = CompletionItem {
        ...
        sort_text: Some(format!("{:03}", sorted_index)),
        insert_text_mode: Some(emmy_lsp_types::InsertTextMode::ADJUST_INDENTATION),
        ..Default::default()
    };
