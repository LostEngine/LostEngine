...
use emmy_lsp_types::{CompletionItem, CompletionTextEdit, TextEdit};
...
pub fn add_modules(
    ...
    prefix_content: &str,
    text_edit_range: Option<emmy_lsp_types::Range>,
) -> Option<()> {
        ...
        if let Some(child_file_id) = child_module_node.file_ids.first() {
            ...
            if child_module_info.is_visible(&version_number) {
                ...
                let completion_item = CompletionItem {
                    label: name.clone(),
                    kind: Some(emmy_lsp_types::CompletionItemKind::FILE),
            ...
            let completion_item = CompletionItem {
                label: name.clone(),
                kind: Some(emmy_lsp_types::CompletionItemKind::FOLDER),
                filter_text: Some(filter_text),
                text_edit,
                ..Default::default()
            };
