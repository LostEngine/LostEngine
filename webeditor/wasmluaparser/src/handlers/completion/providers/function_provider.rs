...
use emmy_lsp_types::{CompletionItem, Documentation};
use std::sync::Arc;
        ...
        if type_decl.is_enum_key() {
            ...
            for member in members {
                ...
                let completion_item = CompletionItem {
                    label,
                    kind: Some(emmy_lsp_types::CompletionItemKind::ENUM_MEMBER),
                    ..Default::default()
                };
...
fn infer_param_list(
    ...
    if names.len() > 1 {
        builder.add_completion_item(CompletionItem {
            ...
            kind: Some(emmy_lsp_types::CompletionItemKind::INTERFACE),
            ..Default::default()
        });
    }
...
fn add_multi_line_union_member_completion(
    ...
    for (union_sub_typ, description) in union_typ.get_unions() {
        ...
        let label_details =
            description
                .as_ref()
                .map(|description| emmy_lsp_types::CompletionItemLabelDetails {
                    detail: None,
...
fn add_lambda_completion(builder: &mut CompletionBuilder, func: &LuaFunctionType) -> Option<()> {
    ...
    let completion_item = CompletionItem {
        label,
        kind: Some(emmy_lsp_types::CompletionItemKind::FUNCTION),
        sort_text: Some("0".to_string()),
        insert_text: Some(insert_text),
        insert_text_format: Some(emmy_lsp_types::InsertTextFormat::SNIPPET),
        ..Default::default()
    };
